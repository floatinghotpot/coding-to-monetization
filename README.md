...
即使你和我一样，掌握了若干种编程语言、用过数十种开发工具，积累了编程、架构、项目管理、带领研发团队、跨行业领域的经验，也未必知道软件怎样能够直接带来经济回报。

从需求、架构、设计、编码、测试、发布、收款、推广、用户支持、维护，这才是软件产生经济回报的完整闭环。但绝大多数的开发者，仅仅关注研发这个环节，甚至仅仅关注在编码这个环节，这是非常局限的。
...
我认识一个巴西人，他用 HTML5 技术实现的小游戏，每天的活跃用户大约是 10万，广告收益已经达到每个月 15万，从而实现了财务自由。

谷歌在 AppStore 上发布有一个 AdSense 的 APP，可以用这个 APP 每天查看你的 AdMob 收益。
